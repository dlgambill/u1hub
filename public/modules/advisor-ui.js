...
(function () {
  ...
  function buildSettings() {
    ...
    SET.innerHTML =
      ...
      'Two buttons, one key. On the <b>Models</b> tab, <b>✦ Settings</b> on a 3MF card has Claude look at the plate render saved in the file, geometry the Hub measures from the meshes (size, overhangs, floating undersides, bed contact, painted faces, volume) and the designer\'s own profile, then suggest the slicer settings for the printer you pick - a table you set in Orca before slicing. On the job card, <b>✦ AI pre-flight</b> checks an already-sliced gcode against what is loaded in the printer: wrong material or temperature for a head, an empty head a color needs, a missing prime tower, bed temperature wrong for the plate. You get GO, CHECK or STOP and the reasons.<br><br>' +
      ...
      '<b>What is sent, and when.</b> Only when you press a button. For a gcode: a text brief of a few thousand characters - the file name, the slicer settings Orca wrote into it, the filaments it was sliced for, the object count, and what the Hub has recorded in that printer\'s heads. For a 3MF: the measured numbers, the designer\'s settings, the loadout, and the small plate picture from inside the file. Never the gcode or the meshes themselves, never your printers\' addresses. <a href="#" id="advPreview" style="color:var(--signal)">Show the brief for the selected file</a>; the Models panel has a "what was sent" link of its own.<br><br>' +
      '<b>What it costs.</b> About a cent a review on Claude Sonnet 5, two or three for a 3MF with its picture, half that on Haiku. The same file against the same loadout is answered from a local cache for free.' +
      '</div>' +
