...
const { zipRead, zipEntryContent } = require("./slicing.js");   // pure helpers; the slicing module itself need not be on
...
function infoFromEntries(entries) {
  ...
  const ps = byName.get("Metadata/project_settings.config");
  if (ps) {
    try {
      const j = JSON.parse(zipEntryContent(ps).toString("utf8"));
  ...
  const ms = byName.get("Metadata/model_settings.config");
  if (ms) {
    try {
      const t = zipEntryContent(ms).toString("utf8");
...
function register(ctx) {
  ...
  async function walk() {
    ...
    let dirMtime = 0;
    try { dirMtime = (await fs.promises.stat(c.folder)).mtimeMs; } catch (e) { LAST_ERR = "folder not found: " + c.folder; INDEX = { at: Date.now(), folder: c.folder, dirMtime: 0, items: [], creators: [], missing: true }; return INDEX; }
  ...
  async function index(force) {
    const c = conf();
    if (force || !INDEX || INDEX.folder !== c.folder) return refresh();
    let dirMtime = INDEX.dirMtime;
    try { dirMtime = (await fs.promises.stat(c.folder)).mtimeMs; } catch {}
    if (dirMtime !== INDEX.dirMtime) return refresh();
    if (Date.now() - INDEX.at > INDEX_TTL_MS) refresh();   // stale-while-revalidate
    return INDEX;
  }
  ...
  app.get("/api/models", async (req, res) => {
    ...
    res.json({
      folder: ix.folder, missing: !!ix.missing, error: LAST_ERR, indexed_at: ix.at, refreshing: !!WALKING, truncated: !!ix.truncated,
      total_all: ix.items.length, total: list.length, offset, limit,
      creators: ix.creators,
      items: list.slice(offset, offset + limit),
      orca_found: fs.existsSync(conf().orcaExe)
    });
  });
  ...
  app.post("/api/models/settings", (req, res) => {
    ...
    const next = { ...cur };
    if ("folder" in b) { const f = String(b.folder || "").trim(); if (f) next.folder = f; else delete next.folder; }
    ctx.cfg.models = next;
    if ("orcaExe" in b) {
      const sl = (ctx.cfg.slicer && typeof ctx.cfg.slicer === "object") ? { ...ctx.cfg.slicer } : {};
      const x = String(b.orcaExe || "").trim();
      if (x) sl.orcaExe = x; else delete sl.orcaExe;
      ctx.cfg.slicer = sl;
    }
    ctx.saveConfig();
    INDEX = null;
  ...
  app.get("/api/models/thumb", async (req, res) => {
    ...
    try {
      const e = pickThumb(zipRead(await fs.promises.readFile(p)));
      if (!e) { return res.status(404).end(); }
      const png = zipEntryContent(e);
  ...
  app.get("/api/models/info", async (req, res) => {
    ...
    try {
      const info = { file: req.query.file, size: st.size, mtime: st.mtimeMs, ...infoFromEntries(zipRead(await fs.promises.readFile(p))) };
